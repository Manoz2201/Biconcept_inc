...
import 'package:biconcept/features/auth/presentation/screens/login_screen.dart';
...
class _FakeAuthRepository implements AuthRepository {
  ...
  Future<AppResult<User>> login(String email, String password) async {
    if (loginError != null) return Failure(loginError!);
    return Success(
      User(
        id: '1',
        accountId: 'acc',
...
void main() {
  ...
  testWidgets('shows an error banner for invalid credentials', (tester) async {
    ...
    await tester.enterText(find.byType(TextField).last, 'password12');
    await tester.tap(find.byKey(const Key('login-submit')));
    await tester.pumpAndSettle();
