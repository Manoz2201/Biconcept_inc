export default async ({ req, res }) => {
  const now = new Date();
  const startMonth = 4;
  const startYear = now.getUTCMonth() + 1 >= startMonth ? now.getUTCFullYear() : now.getUTCFullYear() - 1;
  ...
  return res.json({ invoiceNumber: `INV/${fy}/${padded}`, financialYear: fy, sequence });
};
